...
import { GrFavorite, GrProjects } from "react-icons/gr";
import { IoHomeOutline, IoPersonSharp } from "react-icons/io5";
import { TbMail, TbMailbox } from "react-icons/tb";
  ...
  return (
    ...
    >
      <motion.div className="md:sticky md:top-12 md:pr-4" layout layoutRoot>
        ...
        <section className="flex w-full items-center justify-center gap-3 py-2 pl-2 lg:flex lg:w-full lg:items-start">
          {social.map((item) => (
            <NextLink
              href={item.url}
              key={item.name}
        ...
        <motion.div
          className="relative z-50  flex h-12 items-center justify-center"
          initial={{ opacity: 0, scale: 0.98, y: 8 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          transition={{
            type: "spring",
